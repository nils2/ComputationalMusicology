...
```{r}
p8 <- YCData %>%
        ggplot(aes(x = tempo, y = stat(count), fill = Label)) +
        facet_wrap(~Bizzey) +
        scale_fill_manual(values = c("#00204DFF", "#7C7B78FF", "#FFBC00")) +
        geom_histogram(binwidth = 10) +
        theme_light() +
        theme(strip.background = element_rect(fill="#7C7B78FF", color = "NA"),
              legend.position = c(0.13, 0.8)) +
        labs(title = "Tempo distribution", x = "Tempo", y = "Count", subtitle = "Distributed over the time period with and without Bizzey")
```
...
datatable(playlistYC, rownames = FALSE, class = "table1",
          ...
          options = list(scrollY = "400px", pageLength = 120, paging = FALSE))
```
...
In this graph one can hardly distinguish between the two time area of Yellow Claw, c06 and c12 are slightly different after Bizzey left for example. However there is **one big difference**, the variance in c02 which is known as **the loudness brightness of a track**. Here it is visible that loudness has a range of nearly 100 when Bizzey was still in the band. However after his departure the range nearly doubles as it gets close to a range of 200. 
