...
If you are interested you can check out their tracks right [here](https://open.spotify.com/playlist/4ihYAvvARSvL4yUzdibPrd?si=smVI-XMlRRiikn2SXwt1sw) or below. But for now let's try to discover if they have changed.

<iframe src="https://open.spotify.com/embed/playlist/4ihYAvvARSvL4yUzdibPrd" width="250" height="330" frameborder="0" allowtransparency="true" allow="encrypted-media"></iframe>
...
*<span style='color:#7C7B78FF'>This plot shows the difference in speechiness and liveness in the time Bizzey was and wasn't part of the group. By hovering the plot one can explore the data depicted in the graphs.</span>*
...
*<span style='color:#7C7B78FF'>This plot shows the difference in acousticness and liveness in the time Bizzey was and wasn't part of the group. By hovering the plot one can explore the data depicted in the graphs.</span>*
...
From the timbre-based self-similarity matrices a pattern of the song becomes clear. It is visible that from 20 sec to 95 sec in the song the same occurs as from 95 sec to 160 sec. This coincides more with the typical <span style='color:#FFD200'><b>Yellow Claw</b></span> style of some repetition in their song, than was the case with Loudest MF.
...
Both the timbre-based and chroma-based self-similarity matrices visualize a very repetitive track. It is visible that from 45 sec to 95 sec in the song the same occurs as from 125 sec to 175 sec. But also from 20 sec to 45 sec the track is repeated in 95 sec to 125 sec. This also coincides more with the typical <span style='color:#FFD200'><b>Yellow Claw</b></span> style of some repetition in their song, than was the case with Loudest MF.
...
*<span style='color:#7C7B78FF'>In this section two graphs are computed using lower-level track audio analysis from Spotify. The distinction is again made between the era where Bizzey was a member and the era where he wasn't.</span>*
...
Yet again one can hardly distinguish between the two time era of <span style='color:#FFD200'><b>Yellow Claw</b></span> and only a few coefficients are slightly different after Bizzey left for example. These differences are also to small that no conclusions can be drawn from them. However one could argue there is **one difference**, namely **in the C pitch**. It is visible that the range, mean and deviation of this pitch are different after Bizzey left. 
...
p10 <- YCData %>%
        ...
        theme(strip.background = element_rect(fill="#7C7B78FF", color = "NA")) +
        guides(color = FALSE, fill = FALSE) +
        labs(title = "Loudness distribution", y = "Loudness", x = "Amount") +
...
After Bizzey left <span style='color:#FFD200'><b>Yellow Claw</b></span> had a significant shift in tempo distribution. It's apparent that both distributions are left skewed, however with more so then without. This is because as is visible in the graph tracks were made with tempo above the 150 bpm. And not just a few tracks, almost half of their new music would be above this previous apparent threshold. 
...
### **Analyzing the differences** {data-commentary-width=800}
